#!/usr/bin/env python3
"""
生物杀灭/消毒剂抵抗因子注释结果格式转换工具
将生物杀灭/消毒剂抵抗因子合并文件转换成多种统计格式，便于下游分析与可视化。

用法：
    python3 2-biocide_format_conversion.py <input_csv> <output_dir> [是否输出具体文件]
...
输出：
    0-biocide_sample_totals.csv      # 样本生物杀灭抵抗因子总数
    1-biocide_presence_absence.csv   # 样本 × 生物杀灭/消毒剂抵抗因子 presence/absence 矩阵
    2-biocide_tidy_long.csv          # tidy 长表
    3-biocide_sample_summary.csv     # 样本层面统计
    4-biocide_gene_cooccurrence.csv  # 因子共现矩阵
    5-biocide_gene_summary.csv       # 因子出现频率
...
def export_sample_total_counts(df, output_dir):
    ...
    totals = (
        ...
        .size()
        .reset_index(name="Total_Biocide_Factors")
        .sort_values("Sample")
    )
...
def export_long_format(df, output_dir):
    ...
    long_df.rename(
        columns={
            ...
            "Subject_ID": "Biocide_ID",
            "Subject_Reference": "Reference",
            "Raw_Subject": "Original_sseqid",
        },
        inplace=True,
    )
    long_df.sort_values(["Sample", "Biocide_ID", "Query_locus"], inplace=True)
...
def export_sample_summary(df, output_dir):
    summary = (
        ...
        .agg(
            ...
            Unique_biocide=("Subject_ID", pd.Series.nunique),
...
def export_gene_summary(df, output_dir):
    ...
    gene_df = gene_df[gene_df["Subject_ID"].str.len() > 0]
    if gene_df.empty:
        print("⚠️ 没有可汇总的生物杀灭/消毒剂抵抗因子，跳过 gene summary。")
        return None, None
    summary = (
...
建议流程：
  1. 使用 3-biocide_sample_summary.csv 快速了解每个基因组的生物杀灭抵抗负载。
  2. 使用 1-biocide_presence_absence.csv 进行聚类或热力图分析。
  3. 使用 4-biocide_gene_cooccurrence.csv / 6-biocide_network.json 探索因子共现关系。
"""
    report_file = output_dir / "README_biocide_summary.txt"
