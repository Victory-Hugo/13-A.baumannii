...
# 变更点：
# 1) 使用prokka注释的编码序列（.ffn文件）而非原始组装序列
# 2) 自动识别 /mnt/d/1-鲍曼菌/注释prokka/ 下所有样本的 .ffn文件
# 3) 优先使用 GNU parallel；当 PARALLEL_JOBS==1 或未装 parallel 时回退 for 循环
# 4) 功能其它保持不变
...
mapfile -d '' -t FFN_FILES < <(find "${INPUT_DIR}" -type f -name "*.ffn" -print0)

if [ "${#FFN_FILES[@]}" -eq 0 ]; then
  echo "未在 ${INPUT_DIR} 中发现 .ffn 文件，脚本结束。"
  exit 0
fi
...
run_one_sample() {
  ...
  # OCL
  echo "  - 分析OCL类型..."
  kaptive assembly ab_o "${ffn_file}" \
  ...
  # K locus
  echo "  - 分析K locus类型..."
  kaptive assembly ab_k "${ffn_file}" \
...
for f in "${OUTPUT_DIR}/OCL_results/"*_OCL_results.tsv; do
  # 提取样本名（去掉路径和_OCL_results.tsv后缀）
  sample_name=$(basename "$f" "_OCL_results.tsv")
  # 只保留完全匹配样本名的行（不包含_temp等后缀）
  tail -n +2 "$f" | awk -v sample="$sample_name" '$1 == sample {print; exit}' >> "${OCL_ALL}"
done
...
for f in "${OUTPUT_DIR}/K_locus_results/"*_K_locus_results.tsv; do
  # 提取样本名（去掉路径和_K_locus_results.tsv后缀）
  sample_name=$(basename "$f" "_K_locus_results.tsv")
  # 只保留完全匹配样本名的行（不包含_temp等后缀）
  tail -n +2 "$f" | awk -v sample="$sample_name" '$1 == sample {print; exit}' >> "${K_ALL}"
done
shopt -u nullglob
